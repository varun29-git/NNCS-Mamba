...
- **Mamba SSM Controller (`mamba_controller.py`)**: A neural controller integrating `mamba-ssm`. It achieves native parallel scan scaling during trajectory training sequences and shifts to constant $O(1)$ recurrent sequence processing using `InferenceParams` during online tracking. 
